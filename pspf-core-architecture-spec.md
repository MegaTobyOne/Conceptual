...
- Core acquires an atomic directory lock at `.pspf/core/locks/writer-v2.lock` using only built-in Node filesystem APIs. Runtime acquisition, inspection, reclamation, and release do not invoke a shell, WSL, a native helper, or a timestamp heartbeat.
...
- A live-PID guard prevents takeover of a running owner. A genuinely abandoned lock is quarantined and reclaimed only after its recorded PID is no longer alive; concurrent contenders still rely on atomic directory creation so exactly one writer wins.
- Core releases ownership on clean extension deactivation. Missing or unreadable ownership metadata fails closed.
- External timestamp changes are ignored because timestamps are not lock authority. This permits restricted, synchronised, indexed, backed-up, and security-scanned workspaces without weakening live-owner exclusion.
