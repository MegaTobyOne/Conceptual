...
2. Open Requirement Detail and review the plain-language explainer (What this means / Why it matters / What to do next, ADR 0096 E1), statement, implementation guidance, source authority, current assessment, rationale, linked evidence, linked actions, linked risks, Directions, applied tags, and history. The editable Requirement workbench (ADR 0096 E4, v1.66.0) is organised assess → justify → act: assessment fields, then the consequence/explainer justification, then the linking workbench (create or link existing evidence/action/risk) and its Advanced disclosures. Saving states what changed, what evidence/actions/risks are affected, the current material-risk signal, and a stated next action or stated reason none is needed (ADR 0096 E5, v1.67.0, `buildSaveImpactSummary`).
