...
The programme specification is the 2026-09-19 commitment-led operating model technical specification (`.hermes/plans/2026-09-19_091633-commitment-led-operating-model-techspec.md`, status aspirational). Its repository observations were verified against `develop` on 2026-09-19. [ADR 0099](adr/0099-commitment-led-operating-model-phase-0-baseline.md) is the accepted Phase 0 decision record; the detailed phase plan is [docs/commitment-operating-model-plan.md](docs/commitment-operating-model-plan.md). Phase 1 (explicit working-context presentation plus the `planningState`/history semantic-debt fixes) is in progress; the Core history write-boundary sub-slice is implemented and the working-context/planningState work remains.
...
Status: **implemented (E0–E8 shipped) — governed by ADR 0096**
...
| E1  | 1.63.0  | Plain-language content layer      | Reference-data explainer pack (AU English, attributed, publication-declared) plus one shared explainer renderer; wired to requirement detail in both surfaces. **Shipped**: `REQUIREMENT_EXPLAINERS` curated for all 24 PSPF sections plus `buildRequirementExplainer()` in `@pspf/reference-data`, wired into Explorer `requirement-view.ts` and Workshop's Requirement Detail panel; `check:banned-jargon` and `check:explainer-content` gate the plain-language copy and section coverage.                                                                                                                                                                                                                                                                                                                                                  |
| E2  | 1.64.0  | Requirement finder — Explorer     | Shared contracts search/browse primitive with deterministic ordering; Explorer finder route with visible filters, section browsing, and result decision summaries. **Shipped**: `searchRequirements`/`matchesRequirementFinderFilters`/`compareRequirementFinderRecords`/`buildRequirementFinderResultSummary` in `@pspf/contracts` (generalises Workshop's `savedViewMatchesRequirement`/`compareRequirementsForPicker`); wired into the existing Explorer `/requirements` route (no new route, per the frozen essentials-surface budget) with text search, PSPF section browsing, and per-result decision summaries; `check:requirement-finder` gates the wiring.                                                                                                                                                                            |
| E3  | 1.65.0  | Requirement finder — Workshop     | Workshop pickers backed by the same primitive; parity test proves identical results for identical fixtures across both hosts. **Shipped**: `savedViewMatchesRequirement`/`compareRequirementsForPicker` (extension.ts) now delegate to `matchesRequirementFinderFilters`/`compareRequirementFinderRecords` via a new `requirementToFinderRecord()` adapter in `workshop-ui.ts`; `requirement-finder-parity.test.ts` proves deterministic, canonical-domain-order-invariant results for a shared fixture; `check:requirement-finder-parity` gates the wiring.                                                                                                                                                                                                                                                                                   |
| E4  | 1.66.0  | Decision and action flow          | Requirement detail restructured plain-language-first (assess → justify → act); evidence/action/risk linking in the guided path; advanced fields behind disclosure. **Shipped**: both hosts render Assess → Justify (consequence + explainer) → Act in fixed DOM order (no lens-conditional reordering); Explorer gained a `<pspf-disclosure>` component (Essential Eight/references) and create-new action/risk sub-forms in the linker section (full parity with Workshop's create-vs-link-existing buttons); Workshop's `renderRequirementEditor` gained the same Justify section; `check:requirement-decision-flow` gates the ordering and wiring.                                                                                                                                                                                          |
| E5  | 1.67.0  | Save and impact feedback          | Compact post-save confirmation composed from existing builders; every material change leads to a stated next action or a stated reason none is needed. **Shipped**: `buildSaveImpactSummary()` in `@pspf/contracts` (composes `buildConsequenceStatement` + caller-restated linked-record counts/material-risk state, no new prediction), covering saved/no-op/failed outcomes; wired as a dismissible `<pspf-toast>` banner in Explorer's compliance editor and a Workshop information message on Requirement save; `check:save-impact-feedback` gates the wiring.                                                                                                                                                                                                                                                                            |
| E6  | 1.68.0  | Explorer subtraction              | Retired routes removed; remaining specialist routes under one Advanced group; lens retirement with deterministic preference migration; essentials nav ≤ 7 items. **Shipped**: `/map`, `/map-3d-concepts`, and `/grc` routes and their views (`relationship-map-view.ts`, `connected-view-3d-lab.ts`, `grc-view.ts`) removed, re-baselining to 24 Explorer routes; `NAV_ROUTES` grouped into exactly two groups (`essentials`, `advanced`) with essentials at exactly 7 items (Home, Requirements, Risks, Actions, Directions, Relationships, Posture); presentation lens fully retired in Explorer, Workshop, and `@pspf/webview-shell` — `normalisePresentationLens()` always returns `ciso` and clears any stored legacy value once; `check:essentials-surface` gates route/nav counts, group classification, and the essentials ≤ 7 budget. |
| E7  | 1.69.0  | Workshop subtraction              | Retired panels removed; specialist panels and commands demoted; palette dedupe verified; essentials command set documented and gated. **Shipped**: `openHumanCentredRiskView`, `openContinuousComplianceMetro`, and `openUnifiedSecurityOperatingModel` removed entirely (function, command registration, `package.json` contributions, and their `continuous-compliance.ts` model builders), re-baselining Workshop to 71 commands / 29 webview panels; `docs/workshop-essentials-commands.md` documents the essentials (24) vs specialist (47) split with no palette visibility change; `check:workshop-essentials-commands` gates the documentation and asserts no duplicate command-palette titles across Core, Assurance, Workshop, Shop, and Pub.                                                                                        |
| E8  | 1.70.0  | Release hardening and walkthrough | End-to-end journey gate passes; accessibility, AU-English, and publication scans cover all new copy; `e2e:v1.70` chains continuously from `e2e:v1.61`. **Shipped**: new Playwright journey "assess, evidence, action, save, and see the effect" proves search-by-id → explainer → evidence + action linking → save → visible save-impact banner without touching any Advanced control; the shared requirement finder now also matches on requirement id (previously title/text only); `check-accessibility.mjs`'s route list re-synced to the post-E6 essentials nav (added `/directions`); all Essentials programme gate text in this document and the grand-plan status header flipped from "planned" to "implemented".                                                                                                                      |
