...
| ---------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `full-replace`   | Require all canonical collections, including empty collections, at the current `schemaVersion`, `bundleVersion`, and `apiVersion`. Reject unknown envelope/collection keys, schema or format failures, duplicate IDs, manifest count/hash mismatches, and unresolved mapping endpoints before any write. Write a durable pre-import checkpoint, then **clear the complete entity store, load the bundle, and record success in one transaction**. Any SQL failure rolls back the deletion and partial inserts; explicit undo restores the checkpoint.                                                                                    |
...
| ---------------------------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------- |
| Publication (`intent: full-replace`)                 | Every canonical collection in `V0_1_COLLECTIONS`, including empty collections, plus the required summary material.                                    |
| Local-authoring full backup (`intent: full-replace`) | Every canonical collection in `V0_1_COLLECTIONS`, including empty collections, exactly one current posture record, and all referenced link endpoints. |
