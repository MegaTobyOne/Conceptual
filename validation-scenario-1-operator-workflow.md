# Validation Scenario 1: v1.43.0 UX, Strategy Priority, Pub Team Planning, and Cyber Reference Dataset Workflow
...
Still out of scope for v1.43.0:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.43.0`, `Schema 1.14.0`, and `API 1.14.0`.
3. Confirm the VS Code status bar shows `PSPF v1.43.0` and its tooltip includes `Schema 1.14.0`, `Bundle 1.14.0`, and `API 1.14.0`.
...
6. Click `Edit evidence` and confirm the queue opens with missing/freshness/unlinked evidence groups and `Urgent Actions (Blocked or Overdue)`.
...
- A readiness report at `.tmp/release-readiness/v1.43.0-readiness-report.md`.
