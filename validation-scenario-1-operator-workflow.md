# Validation Scenario 1: v1.29.2 UX Consistency Workflow
...
Still out of scope for v1.29.2:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.29.2`, `Schema 1.10.0`, and `API 1.10.0`.
3. Confirm the VS Code status bar shows `PSPF v1.29.2` and its tooltip includes `Schema 1.10.0`, `Bundle 1.10.0`, and `API 1.10.0`.
...
30. Open Pub from the Activity Bar or run `PSPF: Open Pub`. Confirm `PSPF Pub v1.29.2`, local-only people context, organisation chart, relationship context, assignments and rotations, and `no Explorer publication in v1.29` are visible. Load the Pub sample if needed, then open People, Roles, Assignments, and Relationship Log. For each local record type, open its detail panel and edit panel, confirm the expected fields are visible, save without implying that Pub data is exported, and confirm Relationship Note detail/edit covers person, recorded date, summary, and next contact.
...
32. Confirm the dry run created no `core/1.29.2`, `workshop/1.29.2`, `shop/1.29.2`, or `pub/1.29.2` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
- A readiness report at `.tmp/release-readiness/v1.29.2-readiness-report.md`.
