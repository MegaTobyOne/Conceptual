# Validation Scenario 1: v1.49.0 Assurance City, Unified Explorer Web Surface, Pub Workforce Decision Cockpit and Assurance Workflow
...
Still out of scope for v1.49.0:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.49.0`, `Schema 1.14.0`, and `API 1.14.0`.
3. Confirm the VS Code status bar shows `PSPF v1.49.0` and its tooltip includes `Schema 1.14.0`, `Bundle 1.14.0`, and `API 1.14.0`.
...
36. Confirm the dry run created no `core/1.49.0`, `workshop/1.49.0`, `shop/1.49.0`, `pub/1.49.0`, or `assurance/1.49.0` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
```sh
npx pnpm@10.10.0 run e2e:v1.49
```
...
- A readiness report at `.tmp/release-readiness/v1.49.0-readiness-report.md`.
