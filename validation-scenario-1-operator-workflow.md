# Validation Scenario 1: v1.44.0 UX, Strategy Priority, Pub Team Planning, and Cyber Reference Dataset Workflow
...
Still out of scope for v1.44.0:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.44.0`, `Schema 1.14.0`, and `API 1.14.0`.
3. Confirm the VS Code status bar shows `PSPF v1.44.0` and its tooltip includes `Schema 1.14.0`, `Bundle 1.14.0`, and `API 1.14.0`.
...
36. Confirm the dry run created no `core/1.44.0`, `workshop/1.44.0`, `shop/1.44.0`, `pub/1.44.0`, or `assurance/1.44.0` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
- A readiness report at `.tmp/release-readiness/v1.44.0-readiness-report.md`.
