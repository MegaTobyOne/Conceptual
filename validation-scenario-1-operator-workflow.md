# Validation Scenario 1: v1.21.0 Shop Forecast and Connected View Workflow
...
- Workshop and Explorer Connected View: compact card labels, hover/focus details, transitive chain highlighting, related Requirement emphasis, toolbar clear, and toolbar refresh.
...
- Shop commercial coverage dashboard: linked/unlinked suppliers, contracts, and spend items; monthly and FY forecast; planned savings schedule; annual planned efficiency dividends; CSV/XLS exports; near-term contract review; funded open Actions; supplier Risk links; and quick actions to existing link commands.
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.21.0`, `Schema 1.8.0`, and `API 1.8.0`.
3. Confirm the VS Code status bar shows `PSPF v1.21.0` and its tooltip includes `Schema 1.8.0`, `Bundle 1.8.0`, and `API 1.8.0`.
...
23. Open Explorer `Connected View` and confirm the compact layout shows Directions, grouped Requirements, Risks, and one Actions lane. Hover a card to check the positioned details popover, select a Requirement to check chain and related-Requirement highlighting, use `Clear selection`, and click `Refresh` to reload the static page.
...
31. Confirm the dry run created no `core/1.21.0`, `workshop/1.21.0`, or `shop/1.21.0` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
```sh
npx pnpm@10.10.0 run e2e:v1.20
```
...
- A readiness report at `.tmp/release-readiness/v1.21.0-readiness-report.md`.
...
- Manual operator validation should focus on the v1.21.0 Shop Forecast monthly/FY forecast, planned savings schedule, annual planned efficiency dividends, CSV/XLS export, supplier management and FOCI prompts, CPR contract artefact links, Connected View in Workshop and Explorer, Shop commercial coverage dashboard, assurance-linkage path, commercial context display, Shop visual identity, compliance-history export path, Marketplace dry-run clarity, Workshop Dashboard/Evidence Review saved views, Change Record flow, tag creation/application/filtering, Explorer tag filter URL/session behaviour, Workshop/Explorer visual identity separation, Workshop import review, plan-apply review, schema-change reload guidance, and undo clarity.

## Pass Criteria

- The 32-step manual operator flow completes without intervention.
...
- Any Connected View hover, selection, related-Requirement highlight, refresh, or compact-card behaviour that felt unclear.
