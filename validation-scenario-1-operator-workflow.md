# Validation Scenario 1: v1.28.2 Pub Marketplace Foundation Workflow
...
Still out of scope for v1.28.2:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.28.2`, `Schema 1.10.0`, and `API 1.10.0`.
3. Confirm the VS Code status bar shows `PSPF v1.28.2` and its tooltip includes `Schema 1.10.0`, `Bundle 1.10.0`, and `API 1.10.0`.
...
30. Open Pub from the Activity Bar or run `PSPF: Open Pub`. Confirm `PSPF Pub v1.28.2`, local-only people context, organisation chart, relationship context, assignments and rotations, and `no Explorer publication in v1.28` are visible. Run each Pub command and confirm it returns to the foundation surface without implying that Pub data is exported.
...
32. Confirm the dry run created no `core/1.28.2`, `workshop/1.28.2`, `shop/1.28.2`, or `pub/1.28.2` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
- A readiness report at `.tmp/release-readiness/v1.28.2-readiness-report.md`.
