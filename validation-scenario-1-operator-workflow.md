# Validation Scenario 1: v1.48.0 Unified Explorer Web Surface, Pub Workforce Decision Cockpit and Assurance Workflow
...
Still out of scope for v1.48.0:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.48.0`, `Schema 1.14.0`, and `API 1.14.0`.
3. Confirm the VS Code status bar shows `PSPF v1.48.0` and its tooltip includes `Schema 1.14.0`, `Bundle 1.14.0`, and `API 1.14.0`.
...
20. Serve the built Explorer (`npx pnpm@10.10.0 --filter pspf-explorer run preview` after `pnpm build`, or open the deployed `/explorer/` URL) and confirm the unified PSPF Explorer app shell loads with the home view showing all six PSPF domains, the version marker, and AU-English copy throughout.
...
24. On a Requirement view, set a compliance state, save a note, and add an evidence reference. Reload the browser and confirm the state, note, and evidence persist as browser-local data. Check the coverage and analytics views reflect the change.
...
36. Confirm the dry run created no `core/1.48.0`, `workshop/1.48.0`, `shop/1.48.0`, `pub/1.48.0`, or `assurance/1.48.0` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
```sh
npx pnpm@10.10.0 run e2e:v1.48
```
...
- A readiness report at `.tmp/release-readiness/v1.48.0-readiness-report.md`.
