# Validation Scenario 1: v1.50.0 Dark-First Ecosystem, Unified Explorer and Assurance Workflow
...
- Dark-first shared identity across Core, Assurance, Workshop, Shop, Pub, and Explorer; VS Code theme inheritance; Explorer Dark, Light, and System preferences; and CISO, Auditor, and Solo IT presentation lenses.
...
Still out of scope for v1.50.0:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.50.0`, `Schema 1.14.0`, and `API 1.14.0`.
3. Confirm the VS Code status bar shows `PSPF v1.50.0` and its tooltip includes `Schema 1.14.0`, `Bundle 1.14.0`, and `API 1.14.0`.
...
20. Serve the built Explorer (`npx pnpm@10.10.0 --filter pspf-explorer run preview` after `pnpm build`, or open the deployed `/explorer/` URL) and confirm the unified PSPF Explorer app shell loads in Dark on first use with the home view showing all six PSPF domains, the version marker, and AU-English copy throughout. Switch to Light and System, reload after each choice, and confirm the preference persists. At 1440 px, 768 px, 390 px, and 320 px, confirm controls and text remain within the viewport without overlap.
...
36. Confirm the dry run created no `core/1.50.0`, `workshop/1.50.0`, `shop/1.50.0`, `pub/1.50.0`, or `assurance/1.50.0` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
- A readiness report at `.tmp/release-readiness/v1.50.0-readiness-report.md`.
