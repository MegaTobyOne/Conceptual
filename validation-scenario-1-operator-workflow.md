# Validation Scenario 1: v1.31.2 6clicks Risk Source Hardening Workflow
...
Still out of scope for v1.31.2:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.34.1`, `Schema 1.11.0`, and `API 1.11.0`.
3. Confirm the VS Code status bar shows `PSPF v1.34.1` and its tooltip includes `Schema 1.11.0`, `Bundle 1.11.0`, and `API 1.11.0`.
...
36. Confirm the dry run created no `core/1.31.2`, `workshop/1.31.2`, `shop/1.31.2`, or `pub/1.31.2` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
- A readiness report at `.tmp/release-readiness/v1.31.2-readiness-report.md`.
...
- Manual operator validation should focus on the v1.31.2 6clicks Risk Source panel, explicit fixture/live mode, credential-free fixture handling, HTTPS live-mode validation, selected apply, redacted local run logs, explicit source-overwrite consent, and publication-safe source metadata, plus the v1.29 UX coverage and relationship-rule foundation, Pub local detail/edit coverage, Pub graphic Organisation Chart, export-format direction for native slide/document artefacts, Marketplace dry-run support, and earlier regression surfaces.
