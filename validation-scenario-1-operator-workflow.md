# Validation Scenario 1: v1.35.0 ISM Obligation Navigation Workflow
...
Still out of scope for v1.35.0:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.35.0`, `Schema 1.12.0`, and `API 1.12.0`.
3. Confirm the VS Code status bar shows `PSPF v1.35.0` and its tooltip includes `Schema 1.12.0`, `Bundle 1.12.0`, and `API 1.12.0`.
...
24a. Open Explorer `Obligations`, confirm PSPF Requirements and ISM Controls are shown in one read-only table, then open `ISM Source Controls` and confirm each row can show mapped Requirements plus direct evidence/action/risk counts without displaying per-control implementation status. Copy the posture brief and confirm it includes `ISM Control Posture` aggregate counts only.
...
36. Confirm the dry run created no `core/1.35.0`, `workshop/1.35.0`, `shop/1.35.0`, or `pub/1.35.0` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
- A readiness report at `.tmp/release-readiness/v1.35.0-readiness-report.md`.
...
- Manual operator validation should focus on v1.35 ISM control saved views, implementation-status navigation, direct control work links, Explorer Obligations review, and public-safe ISM posture brief rows, plus the v1.31 6clicks Risk Source panel, Pub local detail/edit coverage, Marketplace dry-run support, and earlier regression surfaces.
