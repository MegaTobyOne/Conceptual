# Validation Scenario 1: v1.39.0 Cyber Reference Dataset Workflow
...
- Explorer ISM review: read-only Obligations navigation, ISM Source Control rows with mapped Requirements and direct work-link counts, and public-safe ISM Control Posture rows in the copied posture brief.
...
Still out of scope for v1.39.0:
...
2. Open the PSPF Workshop Activity Bar item and confirm `Workshop Home` appears with `PSPF v1.39.0`, `Schema 1.14.0`, and `API 1.14.0`.
3. Confirm the VS Code status bar shows `PSPF v1.39.0` and its tooltip includes `Schema 1.14.0`, `Bundle 1.14.0`, and `API 1.14.0`.
...
23. Open Explorer `Connected View` and confirm the compact layout shows Directions, Requirements, Risks, and one Actions lane. Use zoom in, zoom out, reset, and lane visibility controls. Hover a card to check the positioned details popover, select a Requirement to check selected-chain scroll, chain highlighting, related-Requirement highlighting, and connector lines remaining visually under the selected card, use `Clear selection`, and click `Refresh` to reload the static page.
24. On the Requirements section, switch between `All`, PSPF domain tabs, and `Directions`; confirm each tab narrows the table without changing the underlying records. Select the `Security uplift` tag filter and confirm the URL includes `tags=` and `tagsMode=any`. Switch to `All selected tags`, reload the page, and confirm the tag filter is restored from the URL/session state. Check the Relationships Board uses the same tag filter.
24a. Open Explorer `Obligations`, confirm PSPF Requirements and ISM Controls are shown in one read-only table with public review-state cues, then open `ISM Source Controls` and confirm each row can show mapped Requirements, direct evidence/action/risk counts, and review state without displaying per-control implementation status. Copy the posture brief and confirm it includes `ISM Control Posture` aggregate counts only.
25. In Requirements, combine `Explorer Search`, a Requirement status filter, a domain or `Directions` tab, and the `Security uplift` tag filter. Confirm the filtered-count cue says how many Requirements are shown and `Clear filters` restores the full `All` tab. Save the current Requirements view as `Security uplift focus`, clear the filters, apply the saved view, and confirm the search/status/tag controls and visible rows return without reopening Explorer. Rename the view, archive it, and confirm each change redraws the controls immediately.
...
34. Open Pub from the Activity Bar or run `PSPF: Open Pub`. Confirm `PSPF Pub v1.39.0`, local-only people context, organisation chart, relationship context, assignments and rotations, and local-only Pub publication wording are visible. Load the Pub sample if needed, then open People, Roles, Assignments, and Relationship Log. Confirm the Organisation Chart shows a structure-first graphical team/role/assignment view plus a supporting table for team, parent, role, reporting line, and assigned people, without repeating detailed responsibilities or role outcomes. For each local record type, open its detail panel and edit panel, confirm the expected fields are visible, save without implying that Pub data is exported, and confirm Relationship Note detail/edit covers person, recorded date, summary, and next contact.
...
36. Confirm the dry run created no `core/1.39.0`, `workshop/1.39.0`, `shop/1.39.0`, or `pub/1.39.0` remote receipt tags. Do not approve or run a non-dry-run Marketplace publish as part of this manual validation unless this is the actual release publication window.
...
- Requirements navigation feels quick and natural: domain tabs, the Directions lens, search, status, and tag filters compose predictably, and filtered-count cues make reduced result sets obvious.
...
- A readiness report at `.tmp/release-readiness/v1.39.0-readiness-report.md`.
...
- Manual operator validation should focus on v1.39 cyber reference dataset diagnostics, clean-start reset recovery, reciprocal ISM mapping open/edit actions, v1.36 ISM Review Workbench queues, v1.35 ISM control saved views, implementation-status navigation, direct control work links, Explorer Obligations review, and public-safe ISM posture brief rows, plus the v1.31 6clicks Risk Source panel, Pub local detail/edit coverage, Marketplace dry-run support, and earlier regression surfaces.
