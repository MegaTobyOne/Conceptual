...
54. **v0.9 release-candidate freeze** (ADR 0027). v0.9 adds no new product features. It refreshes the manual validation scenario for the current v1.0 surface, makes `release:readiness` active-version aware, and adds `check:release-candidate` to verify package versions, compatibility axes, scripts, docs, and Shop/Pub deferrals. No schema, bundle, or API change — `VERSION_AXES` stays at `1.3.0`; `PSPF_SLICE_VERSION` bumps to `0.9.0`.
55. **v1.0 initial assurance user testing release** (ADR 0028). v1.0 is a release cut from v0.9 for initial Australian Government assurance user testing. It adds no product features, bumps package versions and `PSPF_SLICE_VERSION` to `1.0.0`, keeps `VERSION_AXES` at `1.3.0`, and uses `e2e:v1.0`, `check:release-candidate`, `release:readiness`, and the manual validation scenario as acceptance evidence.
