...
test('coverage matrix totals reflect persisted compliance', async ({ page }) => {
  ...
  // With nothing recorded, all 217 requirements should be in not-set
  await page
    .locator('pspf-app')
  ...
  await expect(view.locator('[data-grand-total]')).toHaveText('217');
  await expect(view.locator('[data-overall-compliant-pct]')).toHaveText('0%');
  await expect(view.locator('[data-total-state="not-set"]')).toHaveText('217');
});
