...
4. Core writer ownership uses an atomic directory lock at `.pspf/core/locks/writer-v2.lock`. A process-local ownership registry and random ownership token prevent diagnostic metadata from becoming lock authority. A live-PID guard prevents takeover of a running owner. Initialisation and every mutating entry point fail closed when ownership is unavailable.
...
The `1.61.3` hotfix removes `proper-lockfile` and its timestamp heartbeat entirely. Core now acquires, inspects, reclaims, and releases writer ownership with built-in Node filesystem and process APIs only. No WSL, shell, or native helper is required; external timestamp changes are ignored, live-PID exclusion remains fail closed, and abandoned locks are quarantined before atomic reacquisition. The compatibility axes remain `1.15.0`.
