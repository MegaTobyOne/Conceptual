...
**Current implementation note:** v1.61.3 is the active hotfix release candidate. It carries the ADR 0095 security and reliability hardening from v1.61.0, the cross-product malformed persisted-value protection from v1.61.1, and replaces the v1.61.2 timestamp-heartbeat lock with built-in Node filesystem/process ownership requiring no WSL or shell. The v1.53.0–v1.60.0 UX judgement-support programme (ADRs 0087–0094) remains complete. `VERSION_AXES` moved once, at v1.57.0, from `1.14.0` to `1.15.0` and remain there for v1.61.3.

**Status: v1.61.3 implementation is ready for automated release-hardening and manual operator validation. Future Mission Control remains aspirational; Phase A prototype implementation in `docs/mockups/` is ready for facilitated concept testing only and is not part of shipped product scope.**
...
- pnpm workspaces with `@pspf/contracts`, `@pspf/brief-renderer`, `pspf-core`, `pspf-workshop`, and the unified `pspf-explorer` web app.
...
- Explorer: unified Lit/Vite app, bundle load with contract and checksum validation, posture brief view with copy-to-clipboard, editable posture screen backed by browser-local Requirement status overlays, compliance donut, relationship-map board mode, Requirement and Relationships tag filters, Requirements and Relationships saved views, schema-change reload guidance for remembered bundles, OFFICIAL: Sensitive + TLP:AMBER+STRICT banner, active version context, v1.1 browser-local Requirement status overlays, v1.2 browser-local evidence references, v1.3 browser-local Actions, v1.4 browser-local Risks plus local status conflict display, and v1.5.1 latest-bundle refresh restore.
...
`npx pnpm@10.10.0 run release:readiness` is the final automated gate for v1.61.3:
...
The next sequence is manual validation of v1.61.3, with emphasis on writer locking in restricted environments without WSL or shell access, hostile and missing legacy display values across all extension surfaces, competing-window read-only behaviour, full-replace recovery messaging, restored-workspace first write, compact responsive navigation, command access, unified Explorer/Core round-trip behaviour, and the existing UX judgement-support surfaces.

## Review conclusion

The Core, Workshop, unified Explorer publication/local-authoring, ISM mapping, Directions, Action Impact, first-run sample, integrity/readiness spine, Requirement tags, saved views, Workshop import review, Shop commercial planning, optional AI controls, Assurance City, Studio System identity, Pub workforce decision cockpit, the S0–S7 UX judgement-support programme, and ADR 0095 hardening are implemented for v1.61.3 readiness. Manual validation now needs to confirm Node-only locking in the restricted Extension Host, import failure paths, malformed legacy display fallbacks, and the existing compact visual and accessibility matrix.
