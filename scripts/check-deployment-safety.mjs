...
assert.ok(
    (standardBundle.collections.requirements ?? []).some((record) => typeof record.title === "string" && record.title.length > 0),
...
console.log(`ok deployment safety passed for ${checkedBundles.length} publication bundle(s) and ${staticRoots.length} static root(s)`);
...
function validateRecord(filePath, collectionName, index, record) {
    ...
    if (!policy) {
        failures.push(`${relative(root, filePath)} ${collectionName}[${index}] has missing publication policy for entity type ${String(entityType)}`);
        return;
    }
...
function normaliseBundle(value) {
    if (value?.manifest?.bundleType === "pspf-explorer-bundle" && value.collections && typeof value.collections === "object") {
        return value;
    }
    return undefined;
}
