...
async function benchCli(label, dbPath, entityCount) {
  ...
  const batchedSql = [
    "BEGIN IMMEDIATE;",
    ...Array.from({ length: entityCount }, (_, i) => upsertEntitySql(makeEntity(i))),
    "COMMIT;"
  ].join("\n");
...
async function main() {
  ...
  for (const n of sizes) {
    ...
    console.log(
      `${String(n).padEnd(6)} ${cliPer.toFixed(4).padStart(8)}    ${jsPer.toFixed(4).padStart(8)}    ${(cliPer / jsPer).toFixed(2)}x`
    );
  }
  ...
  console.log();
  console.log(
    `sql.js shipping cost: sql-wasm.wasm = ${(wasm.size / 1024).toFixed(0)} KB, sql-wasm.js = ${(jsGlue.size / 1024).toFixed(0)} KB`
  );
}
