...
for (const extensionPackage of extensionPackages) {
  ...
  assert.equal(
    manifest.private,
    true,
    `${extensionPackage.name} remains blocked from npm publishing; Marketplace publishing uses vsce`
  );
  assert.equal(manifest.license, "MIT", `${extensionPackage.name} package declares the MIT licence`);
  assert.equal(
    manifest.repository?.url,
    "git+https://github.com/MegaTobyOne/Conceptual.git",
    `${extensionPackage.name} package declares the repository URL`
  );
  assert.equal(
    manifest.repository?.directory,
    extensionPackage.directory,
    `${extensionPackage.name} package repository directory is accurate`
  );
  assert.equal(
    manifest.publisher,
    "tobyharvey",
  ...
  for (const expectedCommand of extensionPackage.expectedCommands) {
    assert.equal(
      commands.has(expectedCommand),
      true,
      `${extensionPackage.name} manifest contributes ${expectedCommand}`
    );
  }
  if (extensionPackage.name === "Workshop") {
    assert.equal(
      manifest.contributes?.viewsContainers?.activitybar?.some((container) => container.id === "pspfWorkshop"),
      true,
      "Workshop contributes an Activity Bar container"
    );
    assert.equal(
      manifest.contributes?.views?.pspfWorkshop?.some(
        (view) => view.id === "pspfWorkshop.homeView" && view.type === "webview"
      ),
      true,
  ...
  if (extensionPackage.name === "Shop") {
    assert.equal(
      ...
      true,
      "Shop contributes an Activity Bar container"
    );
    assert.equal(
      manifest.contributes?.views?.pspfShop?.some(
        (view) => view.id === "pspfShop.forecastView" && view.type === "webview"
      ),
      true,
      "Shop contributes the Forecast webview view"
    );
    assert.equal(
      manifest.contributes?.views?.pspfShop?.some((view) => view.id === "pspfShop.suppliersView"),
      true,
      "Shop contributes the Suppliers tree view"
    );
    assert.equal(
      manifest.contributes?.views?.pspfShop?.some((view) => view.id === "pspfShop.contractsView"),
      true,
      "Shop contributes the Contracts tree view"
    );
    assert.equal(
      manifest.contributes?.views?.pspfShop?.some((view) => view.id === "pspfShop.spendView"),
      true,
  ...
  if (extensionPackage.dependency) {
    assert.equal(
      ...
      true,
      `${extensionPackage.name} declares Core extension dependency`
    );
  }
}
