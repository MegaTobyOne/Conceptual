...
assert.equal(
  coreBundle.includes("const require = __pspfCreateRequire(import.meta.url)"),
  true,
  "Core bundle defines CommonJS require for sql.js"
);
assert.equal(
  coreBundle.includes("const __filename = __pspfFileURLToPath(import.meta.url)"),
  true,
  "Core bundle defines __filename for sql.js"
);
assert.equal(
  coreBundle.includes("const __dirname = __pspfDirname(__filename)"),
  true,
  "Core bundle defines __dirname for sql.js"
);
...
`,
  ...
  const output = execFileSync(process.execPath, [outputPath], {
    cwd: root,
    encoding: "utf8",
...
`,
  ...
  const extensionSmokeOutput = execFileSync(process.execPath, [extensionSmokePath], {
    cwd: root,
    encoding: "utf8",
    stdio: ["ignore", "pipe", "pipe"]
  }).trim();
