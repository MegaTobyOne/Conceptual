...
try {
  ...
  await page.waitForFunction(
    () =>
      typeof globalThis.pspfExplorerRender === "function" && document.body.innerText.includes("Reload your PSPF JSON")
  );
  ...
  const requirement =
    bundle.collections.requirements.find((item) => item.assessmentStatus !== "met") ||
    bundle.collections.requirements[0];
  const finderTargetRequirement =
    bundle.collections.requirements.find((item) => item.id !== requirement.id) || requirement;
  ...
  await page.waitForFunction(
    ...
    finderTargetRequirement.title
  );
  const finderExactMatch = await page.evaluate(
    (expectedId) => ({
      filteredVisible: Array.from(document.querySelectorAll(".local-requirement-option")).filter(
  ...
  const finderNoMatch = await page.evaluate(() => ({
    ...
    selectedStillOpen:
      document
        .querySelector('[aria-labelledby="local-selected-heading"]')
        ?.textContent?.includes("No Requirement selected") === false
  }));
  await page.locator("#local-clear-search").click();
  await page.waitForFunction(
    (expectedRows) =>
      document.querySelector("#explorer-search")?.value === "" &&
      document.querySelectorAll(".local-requirement-option:not([hidden])").length === expectedRows,
    initialVisibleRequirements
  );
  ...
  const linkedContextOpenButton = await page
    .locator("#evidence")
  ...
  const localAuthoringOpenAfterStatusSave = await page
    .locator("#local-authoring")
    .evaluate((section) => section instanceof HTMLDetailsElement && section.open);
  ...
  await page.evaluate(
    async ({ requirementId }) => {
      await globalThis.pspfExplorerAddLocalEvidenceReference(
        requirementId,
        "Local test evidence",
        "https://example.gov.au/evidence/local-test"
      );
    },
    { requirementId: requirement.id }
  );
  await page.waitForFunction(() =>
    document.querySelector("#local-authoring")?.textContent?.includes("Local evidence references: 1")
  );
  const localAuthoringOpenAfterEvidenceSave = await page
    .locator("#local-authoring")
    .evaluate((section) => section instanceof HTMLDetailsElement && section.open);
  ...
  await page.waitForFunction(() =>
    document.querySelector("#local-authoring")?.textContent?.includes("Local actions: 1")
  );
  const localAuthoringOpenAfterActionSave = await page
    .locator("#local-authoring")
    .evaluate((section) => section instanceof HTMLDetailsElement && section.open);
  ...
  const localAuthoringOpenAfterRiskSave = await page
    .locator("#local-authoring")
    .evaluate((section) => section instanceof HTMLDetailsElement && section.open);
  ...
  await page.evaluate(() => {
    ...
    const tagInput = document.querySelector(
      '#requirements .tag-filter-checkbox[value="TAG-00000000-0000-4000-8000-00000000A101"]'
    );
  ...
  await page.waitForFunction((requirementId) => {
    const select = document.querySelector(`select[data-requirement-id="${requirementId}"]`);
    return (
      select?.value === "met" && document.querySelector("#local-authoring")?.textContent?.includes("Local risks: 1")
    );
  }, requirement.id);
  const refreshPersistence = await page.evaluate(
    (requirementId) => ({
      ...
      evidenceVisible: document
        .querySelector("#local-authoring")
        ?.textContent?.includes("Local evidence references: 1"),
  ...
  const savedViewApply = await page.evaluate(async (savedViewId) => {
    ...
    return {
      ...
      visibleRequirements: Array.from(document.querySelectorAll("#requirements tbody tr")).filter((row) => !row.hidden)
        .length,
      statusChecked: document.querySelector('.requirement-status-filter-checkbox[value="met"]')?.checked === true,
      tagChecked:
        document.querySelector('.tag-filter-checkbox[value="TAG-00000000-0000-4000-8000-00000000A101"]')?.checked ===
        true
    };
  ...
  const conflictVisible = await page.evaluate(
    async (value) => {
      ...
      return (
        section?.textContent?.includes("Local status conflicts: 1") &&
        section?.textContent?.includes("Baseline changed")
      );
    },
  ...
  const persisted = await page.evaluate(
    async (value) => {
      ...
      const exportedLink = exported.collections.links.find(
        (item) =>
          item.fromId === value.requirementId && item.toId === exportedEvidence?.id && item.linkType === "supported-by"
      );
      const exportedAction = exported.collections.actions.find((item) => item.title === "Local follow-up action");
      const exportedActionLink = exported.collections.links.find(
        (item) =>
          item.fromId === value.requirementId && item.toId === exportedAction?.id && item.linkType === "addressed-by"
      );
      const exportedRisk = exported.collections.risks.find((item) => item.title === "Local test risk");
      const exportedRiskLink = exported.collections.links.find(
        (item) =>
          item.fromId === value.requirementId && item.toId === exportedRisk?.id && item.linkType === "exposed-by"
      );
      ...
      return {
        ...
        complianceHistoryManifestEntry: exported.manifest.collections.some(
          (collection) => collection.name === "compliance-events"
        ),
        complianceHistoryCount: exported.collections["compliance-events"]?.length || 0,
        complianceHistoryExcluded: !Object.prototype.hasOwnProperty.call(
          exportedWithoutHistory.collections,
          "compliance-events"
        ),
        complianceHistoryManifestExcluded: !exportedWithoutHistory.manifest.collections.some(
          (collection) => collection.name === "compliance-events"
        ),
  ...
  const resetEvidenceCount = await page.evaluate(() =>
    document.querySelector("#local-authoring")?.textContent?.includes("Local evidence references: 0")
  );
  const resetActionCount = await page.evaluate(() =>
    document.querySelector("#local-authoring")?.textContent?.includes("Local actions: 0")
  );
  const resetRiskCount = await page.evaluate(() =>
    document.querySelector("#local-authoring")?.textContent?.includes("Local risks: 0")
  );
  ...
  const checks = [
    ...
    check(
      "Stale schema notice asks for JSON reload",
      staleSchemaNotice.visible &&
        staleSchemaNotice.text.includes("Reload your PSPF JSON") &&
        staleSchemaNotice.text.includes("Select your latest"),
      staleSchemaNotice.text
    ),
    ...
    check(
      "Explorer Search filters Local Changes list",
      finderExactMatch.filteredVisible > 0 &&
        (initialVisibleRequirements === 1 || finderExactMatch.filteredVisible < initialVisibleRequirements) &&
        finderExactMatch.filterBannerVisible,
      `${finderExactMatch.filteredVisible}/${initialVisibleRequirements}`
    ),
    check(
      "Explorer Search moves Local Changes workspace",
      finderExactMatch.filteredVisible === 1 &&
        finderExactMatch.retainedQuery &&
        finderExactMatch.statusText.includes("Local Changes"),
      `visible=${finderExactMatch.filteredVisible}`
    ),
    check(
      "Explorer Search pins selected Local Changes item outside results",
      finderNoMatch.filteredVisible === 1 &&
        finderNoMatch.emptyVisible &&
        finderNoMatch.pinnedVisible &&
        finderNoMatch.selectedStillOpen,
      `visible=${finderNoMatch.filteredVisible}`
    ),
    check(
      "Local Changes clear search restores picker",
      localClearSearch.queryCleared && localClearSearch.bannerCleared && localClearSearch.selectedPinnedCleared,
      JSON.stringify(localClearSearch)
    ),
    check(
      "Local Requirement click selects workspace",
      clickSelection.selectedId === requirement.id,
      clickSelection.selectedId || "missing"
    ),
    ...
    check(
      "Linked Context exposes Open buttons",
      baselineLinkedContext.openButtonCount >= 4,
      `${baselineLinkedContext.openButtonCount} button(s)`
    ),
    ...
    check(
      "Refresh restores local status",
      refreshPersistence.selectedStatus === "met",
      refreshPersistence.selectedStatus || "missing"
    ),
    ...
    check(
      "Saved view captures tag filter",
      savedViewBeforeRefresh.tagIds.includes("TAG-00000000-0000-4000-8000-00000000A101"),
      savedViewBeforeRefresh.tagIds.join(", ")
    ),
    check(
      "Saved view captures status filter",
      savedViewBeforeRefresh.statuses.includes("met"),
      savedViewBeforeRefresh.statuses.join(", ")
    ),
    check(
      "Saved view applies status and tag filters",
      savedViewApply.statusChecked && savedViewApply.tagChecked && savedViewApply.visibleRequirements > 0,
      JSON.stringify(savedViewApply)
    ),
    check(
      "Relationship saved view has Explorer scope",
      relationshipSavedViewBeforeRefresh.scope === "explorer-relationships",
      relationshipSavedViewBeforeRefresh.scope || "missing"
    ),
    check(
      "Relationship saved view captures tag filter",
      relationshipSavedViewBeforeRefresh.tagIds.includes("TAG-00000000-0000-4000-8000-00000000A101"),
      relationshipSavedViewBeforeRefresh.tagIds.join(", ")
    ),
    check(
      "Relationship saved view captures board columns",
      relationshipSavedViewBeforeRefresh.columns.includes("relationship") &&
        relationshipSavedViewBeforeRefresh.columns.includes("from"),
      relationshipSavedViewBeforeRefresh.columns.join(", ")
    ),
    check(
      "Relationship saved view applies tag filter",
      relationshipSavedViewApply.linksOpen &&
        relationshipSavedViewApply.tagChecked &&
        relationshipSavedViewApply.visibleRelationships > 0,
      JSON.stringify(relationshipSavedViewApply)
    ),
    ...
    check(
      "Exports complete master collection set",
      persisted.collectionCount === 14,
      `${persisted.collectionCount} collection(s)`
    ),
    ...
    check(
      ...
      persisted.complianceHistoryIncluded &&
        persisted.complianceHistoryManifestEntry &&
        persisted.complianceHistoryCount === 1,
      `${persisted.complianceHistoryCount} event(s)`
    ),
    check(
      "Compliance history can be excluded",
      persisted.complianceHistoryExcluded && persisted.complianceHistoryManifestExcluded,
      "excluded export"
    ),
    check(
      "Exported requirement carries local status",
      persisted.exportedStatus === "met",
      persisted.exportedStatus || "missing"
    ),
    check(
      "Exported local status source is Explorer",
      persisted.exportedSourceProduct === "explorer",
      persisted.exportedSourceProduct || "missing"
    ),
    check(
      "Exported local evidence reference present",
      persisted.exportedEvidenceReference === "https://example.gov.au/evidence/local-test",
      persisted.exportedEvidenceReference || "missing"
    ),
    check(
      "Exported local evidence source is Explorer",
      persisted.exportedEvidenceSourceProduct === "explorer",
      persisted.exportedEvidenceSourceProduct || "missing"
    ),
    check(
      "Exported local evidence link present",
      persisted.exportedLinkType === "supported-by",
      persisted.exportedLinkType || "missing"
    ),
    check(
      "Exported local evidence link source is Explorer",
      persisted.exportedLinkSourceProduct === "explorer",
      persisted.exportedLinkSourceProduct || "missing"
    ),
    check(
      "Exported local action present",
      persisted.exportedActionStatus === "todo",
      persisted.exportedActionStatus || "missing"
    ),
    check(
      "Exported local action due date present",
      persisted.exportedActionDueDate === "2026-06-30",
      persisted.exportedActionDueDate || "missing"
    ),
    check(
      "Exported local action source is Explorer",
      persisted.exportedActionSourceProduct === "explorer",
      persisted.exportedActionSourceProduct || "missing"
    ),
    check(
      "Exported local action link present",
      persisted.exportedActionLinkType === "addressed-by",
      persisted.exportedActionLinkType || "missing"
    ),
    check(
      "Exported local action link source is Explorer",
      persisted.exportedActionLinkSourceProduct === "explorer",
      persisted.exportedActionLinkSourceProduct || "missing"
    ),
    check(
      "Exported local risk present",
      persisted.exportedRiskStatus === "open",
      persisted.exportedRiskStatus || "missing"
    ),
    check(
      "Exported local risk likelihood present",
      persisted.exportedRiskLikelihood === 4,
      String(persisted.exportedRiskLikelihood || "missing")
    ),
    check(
      "Exported local risk impact present",
      persisted.exportedRiskImpact === 5,
      String(persisted.exportedRiskImpact || "missing")
    ),
    check(
      "Exported local risk source is Explorer",
      persisted.exportedRiskSourceProduct === "explorer",
      persisted.exportedRiskSourceProduct || "missing"
    ),
    check(
      "Exported local risk link present",
      persisted.exportedRiskLinkType === "exposed-by",
      persisted.exportedRiskLinkType || "missing"
    ),
    check(
      "Exported local risk link source is Explorer",
      persisted.exportedRiskLinkSourceProduct === "explorer",
      persisted.exportedRiskLinkSourceProduct || "missing"
    ),
    check(
      "Exported saved view present",
      persisted.exportedSavedViewScope === "explorer-requirements",
      persisted.exportedSavedViewScope || "missing"
    ),
    check(
      "Exported saved view status filter present",
      persisted.exportedSavedViewStatus === "met",
      persisted.exportedSavedViewStatus || "missing"
    ),
    check(
      "Exported saved view tag filter present",
      persisted.exportedSavedViewTag === "TAG-00000000-0000-4000-8000-00000000A101",
      persisted.exportedSavedViewTag || "missing"
    ),
    check(
      "Exported saved view source is Explorer",
      persisted.exportedSavedViewSourceProduct === "explorer",
      persisted.exportedSavedViewSourceProduct || "missing"
    ),
    check(
      "No local data in localStorage",
      persisted.localStorageKeys.length === 0,
      persisted.localStorageKeys.join(", ")
    ),
  ...
  await writeFile(
    join(reportDirectory, "explorer-local-authoring-report.json"),
...
function addBaselineLinkedContext(bundle, requirementId) {
  ...
  bundle.collections.links = [
    ...(bundle.collections.links || []),
    baselineLink(
      "LNK-00000000-0000-4000-8000-00000000A101",
      "Baseline evidence supports requirement",
      requirementId,
      "EVD-00000000-0000-4000-8000-00000000A101",
      "evidence",
      "supported-by",
      timestamp
    ),
    baselineLink(
      "LNK-00000000-0000-4000-8000-00000000A102",
      "Baseline action addresses requirement",
      requirementId,
      "ACT-00000000-0000-4000-8000-00000000A101",
      "action",
      "addressed-by",
      timestamp
    ),
    baselineLink(
      "LNK-00000000-0000-4000-8000-00000000A103",
      "Baseline risk exposes requirement",
      requirementId,
      "RSK-00000000-0000-4000-8000-00000000A101",
      "risk",
      "exposed-by",
      timestamp
    ),
    baselineLink(
      "LNK-00000000-0000-4000-8000-00000000A104",
      "High priority tag marks requirement",
      requirementId,
      "TAG-00000000-0000-4000-8000-00000000A101",
      "tag",
      "tagged-with",
      timestamp
    )
