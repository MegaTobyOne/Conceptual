...
These gates are planned, not yet enforced. Each slice (E0–E8, v1.62.0–v1.70.0) turns its gate live when it ships and chains `e2e:v1.62` … `e2e:v1.70` continuously from `e2e:v1.61`.

1. **Version and compatibility gate**: each slice advances all package versions and `PSPF_SLICE_VERSION` together (E0 = `1.62.0` … E8 = `1.70.0`); `VERSION_AXES` remain `1.15.0` for the entire programme; no entity, link, bundle, API, publication-policy, or Explorer schema change is introduced.
2. **Surface baseline and subtraction gate**: `check:essentials-surface` records the E0 route/panel/command baseline, then fails if a retired view (Explorer Assurance City, Map, or GRC; Workshop Continuous Compliance Metro, Unified Security Model, or Human-Centred Risk) reappears, a demoted view surfaces outside the single Advanced disclosure, or essentials counts exceed the ADR 0096 budget (Explorer essentials navigation ≤ 7 items). No record, entity, command API, or export capability is removed.
...
8. **Release-chain gate**: each slice adds its `e2e:v1.6x` script inheriting the complete previous chain; `release:readiness` targets the newest chain; ADR 0096's coverage entry upgrades from `manual` to `automated` once `check:essentials-surface` and `e2e:v1.62` exist.
