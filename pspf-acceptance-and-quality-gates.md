...
1. **Version and compatibility gate**: all package versions and `PSPF_SLICE_VERSION` are `1.61.3`; `VERSION_AXES` remain `1.15.0`; no entity, link, API, bundle shape, publication policy, or Explorer schema change is introduced.
...
4. **Writer ownership gate**: a process-level test proves exactly one writer, blocks mutation and initialisation in contenders, rejects takeover of a live owner, and safely reclaims an abandoned lock. A behavioural regression changes the active lock timestamp and proves the current writer remains healthy. Package-shape validation rejects `proper-lockfile`, shell-process imports, WSL references, and external lock-library types from Core.
...
### v1.62–v1.70 Essentials programme gates (per ADR 0096, implemented)

All nine gates below are implemented and enforced. `e2e:v1.62` … `e2e:v1.70` chain continuously from `e2e:v1.61`, and each slice's gate went live in the version noted.

1. **Version and compatibility gate**: each slice advances all package versions and `PSPF_SLICE_VERSION` together (E0: all package versions and `PSPF_SLICE_VERSION` are `1.62.0`; E1: all package versions and `PSPF_SLICE_VERSION` are `1.63.0`; E2: all package versions and `PSPF_SLICE_VERSION` are `1.64.0`; E3: all package versions and `PSPF_SLICE_VERSION` are `1.65.0`; E4: all package versions and `PSPF_SLICE_VERSION` are `1.66.0`; E5: all package versions and `PSPF_SLICE_VERSION` are `1.67.0`; E6: all package versions and `PSPF_SLICE_VERSION` are `1.68.0`; E7: all package versions and `PSPF_SLICE_VERSION` are `1.69.0`; E8: all package versions and `PSPF_SLICE_VERSION` are `1.70.0`); `VERSION_AXES` remain `1.15.0` for the entire programme; no entity, link, bundle, API, publication-policy, or Explorer schema change is introduced.
2. **Surface baseline and subtraction gate** (live from E0/v1.62.0; retired-view-reappearance, group-classification, and ≤ 7 essentials-nav-item checks live from E6/v1.68.0): `check:essentials-surface` records the E0 route/panel/command baseline (27 Explorer routes, 20 essentials nav items, 74 Workshop commands, 32 Workshop webview panels) and fails on any further growth. E6 retires the `/map`, `/map-3d-concepts`, and `/grc` Explorer routes (and their views), re-baselines to 24 routes / 18 nav items, and asserts every nav item is grouped `essentials` or `advanced` with the essentials group at exactly 7 items. No record, entity, command API, or export capability is removed.
3. **Plain-language gate** (live from E1/v1.63.0, requirement detail only; other essentials-path surfaces follow in E2-E7): every essentials-path requirement and status surface renders the three-part explainer (_What this means / Why it matters / What to do next_) through the one shared renderer; a banned-jargon lint (terms defined against `pspf-glossary.md`) fails specialist vocabulary on the essentials path; `lint:au` covers all new copy; explainer reference content carries source attribution and declares `publication`. `check:explainer-content` asserts curated coverage of every PSPF baseline section and wiring into Explorer and Workshop requirement detail; `check:banned-jargon` asserts the explainer copy is free of specialist vocabulary.
4. **Finder parity gate** (live from E2/v1.64.0 Explorer-side; full cross-host wiring live from E3/v1.65.0): the `@pspf/contracts` finder primitive (`searchRequirements`, `matchesRequirementFinderFilters`, `compareRequirementFinderRecords`) is the only requirement search/ordering implementation in both hosts — Workshop's `savedViewMatchesRequirement`/`compareRequirementsForPicker` delegate to it via `requirementToFinderRecord()` (`workshop-ui.ts`); result summaries state current assessment, evidence confidence, open actions, and material risk. `check:requirement-finder` (E2) and `check:requirement-finder-parity` (E3) assert the wiring; `requirement-finder-parity.test.ts` proves deterministic, canonical-domain-order-invariant results for a shared fixture.
5. **Decision and action flow gate** (live from E4/v1.66.0): Requirement Detail renders assess → justify → act in that DOM order in both hosts, with no lens-conditional reordering; Explorer's guided path supports linking an existing evidence/action/risk or creating a new one inline (auto-linked to the current requirement); an Advanced disclosure (`<pspf-disclosure>` in Explorer, `disclosureHtml()` in Workshop) holds Essential Eight mapping and references. `check:requirement-decision-flow` asserts the section order and disclosure/create-new wiring in both hosts.
6. **Lens-migration gate** (live from E6/v1.68.0): any stored `ciso`/`auditor`/`solo` lens preference migrates to the default view exactly once and deterministically (`normalisePresentationLens` in `@pspf/webview-shell`, always returning `ciso`); lens-conditional code paths are removed together with their invariance tests in Explorer, Workshop, and `@pspf/webview-shell`; no dead setting remains — the lens selector UI, `decodePresentationLens`/`encodePresentationLens`, and the Workshop `pspf.workshop.home.selectLens` handler are deleted, and any legacy stored value is cleared from `localStorage`/workspace state on first read.
7. **Impact-feedback gate** (live from E5/v1.67.0): the post-save confirmation composes existing posture and impact builders only (`buildSaveImpactSummary`, itself composed from `buildConsequenceStatement` and caller-restated linked-record counts/material-risk state — no new prediction); behavioural tests cover what-changed, affected evidence/actions/risks, and the posture or priority explanation for save, no-op, and failure paths; no prediction or unqualified projection appears. Wired as a dismissible Explorer banner (`<pspf-toast>`) on compliance-status save and a Workshop information message on Requirement save. `check:save-impact-feedback` asserts the wiring.
8. **Journey gate** (live from E8/v1.70.0): an automated end-to-end walkthrough on the standard fixture proves search-or-browse → requirement explainer → evidence and action linking → save → visible posture/risk effect, without touching any Advanced control. `packages/explorer/tests/e2e/ux-evidence-pack.spec.ts`'s "assess, evidence, action, save, and see the effect" test asserts the wiring; `scripts/ux-evidence-pack.mjs` requires exactly 3 recorded journeys. The requirement finder's query now also matches the requirement id (not just title/text), so "search-or-browse" genuinely covers searching by id.
9. **Workshop subtraction gate** (live from E7/v1.69.0): the Human-Centred Risk View, Continuous Compliance Metro, and Unified Security Operating Model Workshop panels and their commands (`pspf.workshop.openHumanCentredRiskView`, `pspf.workshop.openContinuousComplianceMetro`, `pspf.workshop.openUnifiedSecurityOperatingModel`) are removed entirely (function, command registration, and `package.json` contribution), re-baselining Workshop to 71 commands / 29 webview panels; `docs/workshop-essentials-commands.md` documents the full essentials (24) vs specialist (47) command classification with no palette visibility change (`when` clauses are untouched); `check:workshop-essentials-commands` asserts the documentation stays complete and accurate and that no two commands across Core, Assurance, Workshop, Shop, and Pub share an exact palette title; `check:essentials-surface` asserts the three retired commands stay removed.
10. **Release-chain gate** (live from E0/v1.62.0): each slice adds its `e2e:v1.6x`/`e2e:v1.70` script inheriting the complete previous chain; `release:readiness` targets the newest chain; ADR 0096's coverage entry is `automated`.

### v1.71–v1.74 "Brief once, act often" reporting and accountability programme gates (per ADR 0097)

The programme runs four slices (R1–R4), all now implemented. Each slice's gates went live in the version noted; `e2e:v1.71` … `e2e:v1.74` chain continuously from `e2e:v1.70`.

#### v1.71.0 R1 — Reporting Workbench (implemented)

1. **Version and compatibility gate**: all package versions and `PSPF_SLICE_VERSION` are `1.71.0`; `VERSION_AXES` remain `1.15.0`; no entity, link, bundle, API, publication-policy, or Explorer schema change is introduced. The snapshot side file gains a per-record status map only; the master bundle is untouched.
2. **Reporting Workbench gate** (live from R1/v1.71.0): `check:reporting-workbench` asserts that `buildReportingPackModel` in `@pspf/brief-renderer` composes only existing `@pspf/contracts` primitives and `@pspf/reference-data` explainers (no new prediction, no AI prose); that the Workshop `pspf.workshop.openReportingWorkbench` command and Reporting Workbench panel are wired with the Me ↔ All scope toggle (`pspf.workshop.myDomains`), snapshot anchor picker, and Markdown/plain-text copy-out; that the rendered sections appear in the fixed order Executive Brief → Domain packs → Readiness; that every copy-out carries the **OFFICIAL: Sensitive** marking and emits no `Person.name`, `Person.email`, `Assignment.personId`, or restricted field; and that the essentials-surface baseline is re-based exactly once to 72 Workshop commands / 30 Workshop webview panels with the ADR 0097 reason recorded.
3. **Renderer test gate** (live from R1/v1.71.0): `pnpm --filter @pspf/brief-renderer test` covers the reporting pack model for Me and All scope, counts-only degradation when the anchored snapshot predates v1.71.0, and named per-record change when the anchor carries `recordStatus`.
4. **Release-chain gate** (live from R1/v1.71.0): `e2e:v1.71` inherits the complete v1.70 chain and adds `check:reporting-workbench` plus the brief-renderer tests; `release:readiness` targets `e2e:v1.71:run`; ADR 0097's coverage entry is `automated`.

#### v1.72.0 R2 — Schema `1.16.0`: ownership, slippage, narrative (implemented)

1. **Version and compatibility gate**: all package versions and `PSPF_SLICE_VERSION` are `1.72.0`; `VERSION_AXES` move from `1.15.0` to `1.16.0` together (`schemaVersion`, `bundleVersion`, `apiVersion`); `schemas/explorer-bundle/1.16.0/` mirrors `1.15.0/` plus the additive R2 fields and the new `collections/narratives.schema.json`; the standard fixture bundle, generated reference data, and the Core/Explorer compatibility tables record `1.16.0`; `check:schema-coverage` and `check:schema-policy` pass against the new directory.
2. **Ownership schema gate** (live from R2/v1.72.0): `check:ownership-schema` asserts that `action.ownerTeam`, `action.dueDateHistory`, and `requirement.ownerTeam` exist in `@pspf/contracts` and the `1.16.0` schemas with `publication: sensitive`; that the `narrative` entity and `narratives` collection exist with `narrative.body` `sensitive`; that `strategy.owner` declares an explicit publication policy; that Core's `appendDueDateHistory` records every due-date change itself and seeds a legacy entry for pre-1.16 actions on first write; that narrative rule violations raise `PspfError` code `PSPF_NARRATIVE_RULE_VIOLATION`; and that readiness reports the `gap-without-owner` and `action-without-owner` codes.
3. **Legacy workspace forward-compatibility gate** (live from R2/v1.72.0): `pnpm --filter pspf-core test` opens a workspace whose metadata records `1.15.0`, forward-syncs the workspace metadata to `1.16.0`, and preserves every existing record; `pnpm --filter @pspf/contracts test` covers the new field and entity contracts.
4. **Publication gate** (live from R2/v1.72.0): `ownerTeam`, `dueDateHistory`, and `narrative.body` are `sensitive` and never appear in snapshots, export bundles, Explorer artefacts, or copy-outs; `check:explorer-publication` and `check:brief-redaction` pass with the `1.16.0` axes; no field holds `Person.name`, `Person.email`, or `Assignment.personId`.
5. **Workshop authoring gate** (live from R2/v1.72.0): the action and requirement editors expose an owner team field, and the Plan of Action worklist offers a bulk **Assign owner team** action; no new command or webview panel is added, so the essentials-surface baseline stays at 72 commands / 30 panels.
6. **Release-chain gate** (live from R2/v1.72.0): `e2e:v1.72` inherits the complete v1.71 chain and adds `check:ownership-schema`, the `@pspf/contracts` tests, and the `pspf-core` tests; `release:readiness` targets `e2e:v1.72:run`; `check:gates:run` includes `check-ownership-schema.mjs`; ADR 0097's coverage entry remains `automated`.

#### v1.73.0 R3 — Team report card and editable narrative (implemented)

1. **Version and compatibility gate**: all package versions and `PSPF_SLICE_VERSION` are `1.73.0`; `VERSION_AXES` remain `1.16.0`; no entity, link, bundle, API, publication-policy, or Explorer schema change is introduced. The `narrative` entity shipped in R2 is reused unchanged.
2. **Team report card gate** (live from R3/v1.73.0): `check:team-report-card` asserts that `buildTeamReportCard` in `@pspf/contracts` derives one row per distinct `ownerTeam` from existing action and requirement records only; that each row carries gaps owned, actions open, due this period, overdue, slipped, and closed in period, closure velocity, and a deterministic verdict of **On track**, **At risk**, **Stalled**, or **No open work**; that the fixed verdict rules (`TEAM_VERDICT_RULES`) are printed on the card itself alongside every verdict; and that an **Unassigned** row is always present, even when every gap and action has an owner.
3. **Narrative editing gate** (live from R3/v1.73.0): each generated Executive Brief section and Domain pack in the Reporting Workbench offers **Use generated**, **Edit**, and **Restore previous**; an edit persists a `narrative` record whose `supersedesId` points at the record it replaces, so the chain is append-only and restore never deletes; the executive-audience filter limits which narrative slots appear in the Executive Brief tab; `pnpm --filter @pspf/contracts test`, `pnpm --filter @pspf/brief-renderer test`, and `pnpm --filter pspf-workshop test` cover the report card model, the narrative override rendering, and the Workshop panel wiring.
4. **Publication gate** (live from R3/v1.73.0): `narrative.body` remains `sensitive`; operator-authored narrative text appears only in Workshop copy-outs marked **OFFICIAL: Sensitive** and never in snapshots, export bundles, Explorer artefacts, or any executive copy-out that leaves Workshop; the team report card names teams only and never emits `Person.name`, `Person.email`, or `Assignment.personId`; `check:explorer-publication` and `check:brief-redaction` continue to pass.
5. **Workshop surface gate** (live from R3/v1.73.0): the team report card is a tab inside the existing Reporting Workbench panel with a per-team **Copy card** action; no new command or webview panel is added, so the essentials-surface baseline stays at 72 commands / 30 panels.
6. **Release-chain gate** (live from R3/v1.73.0): `e2e:v1.73` inherits the complete v1.72 chain and adds `check:team-report-card`, the `@pspf/contracts` tests, the `@pspf/brief-renderer` tests, and the `pspf-workshop` tests; `release:readiness` targets `e2e:v1.73:run`; `check:gates:run` includes `check-team-report-card.mjs`; ADR 0097's coverage entry remains `automated`.

#### v1.74.0 R4 — Suggested actions, capture sweeps, narrative reuse, and close reporting period (implemented)

1. **Version and compatibility gate**: all package versions and `PSPF_SLICE_VERSION` are `1.74.0`; `VERSION_AXES` remain `1.16.0`; no entity, link, bundle, API, publication-policy, or Explorer schema change is introduced. The `narrative` entity shipped in R2 is reused unchanged.
2. **Narrative reuse gate** (live from R4/v1.74.0): `check:narrative-reuse` asserts that `resolveOperatorNotes` resolves the current head of each `supersedesId` chain for a slot; that the posture brief consumes the resolved narrative for its **Where We Stand** and **What Changed** slots and attributes it as an operator note; that the Digital CISO Magazine sources its editor note from the same resolver (`editorNoteSource`) rather than VS Code `workspaceState`; and that the CISO Master Plan renders the matching slot as `operatorNarrative`. Each consumer prints the operator note beside, never instead of, the deterministic generated sentence and its rule.
3. **Suggested actions gate** (live from R4/v1.74.0): `buildSuggestedActions` in `@pspf/contracts` drafts action stubs only for gaps with no linked action, sourcing wording from the explainer _What to do next_ text, ranking by `rankBlockersByFanIn`, and defaulting `ownerTeam` from the requirement; the Reporting Workbench **Readiness** tab presents them under **Suggested actions** as a draft-and-confirm multi-select, and nothing is written to Core until the operator selects and accepts specific drafts; `pnpm --filter @pspf/contracts test` and `pnpm --filter pspf-workshop test` cover the ranker and the accept flow.
4. **Capture sweeps gate** (live from R4/v1.74.0): the Evidence Review Queue offers bulk **Confirm current**, **Mark stale**, and **Link** operations over the selected evidence rows; the ISM Review Workbench offers a deterministic batch mapping sweep whose drafts come from the existing ranker and are flagged for review before any mapping is persisted; both sweeps live inside their existing panels, so no new command or webview panel is added and the essentials-surface baseline stays at 72 commands / 30 panels.
5. **Close reporting period gate** (live from R4/v1.74.0): **Close reporting period** takes a snapshot and then calls `stampNarrativesWithSnapshot` so every current narrative record carries that snapshot id as `basedOnSnapshotId`; the operation is a single confirmable step, never deletes or rewrites a narrative body, and appends to the supersede chain rather than overwriting.
6. **Publication gate** (live from R4/v1.74.0): `narrative.body` remains `sensitive`; operator notes surfaced in the posture brief, magazine, and master plan appear only in Workshop copy-outs marked **OFFICIAL: Sensitive** and never in snapshots, export bundles, Explorer artefacts, or any artefact that leaves Workshop; suggested action drafts and sweep drafts carry no `Person.name`, `Person.email`, or `Assignment.personId`; `check:explorer-publication` and `check:brief-redaction` continue to pass.
7. **Release-chain gate** (live from R4/v1.74.0): `e2e:v1.74` inherits the complete v1.73 chain and adds `check:narrative-reuse`, the `@pspf/contracts` tests, the `@pspf/brief-renderer` tests, and the `pspf-workshop` tests; `release:readiness` targets `e2e:v1.74:run`; `check:gates:run` includes `check-narrative-reuse.mjs`; ADR 0097's coverage entry remains `automated`.

#### v1.75.0 — Workshop Risk overhaul Phase 1B: schema `1.17.0` baseline (implemented)
...
1. **Version gate**: all package versions and `PSPF_SLICE_VERSION` are `1.41.4`; `VERSION_AXES` remain `1.14.0`; no new Explorer schema directory is introduced.
