...
4. **Finder parity gate** (live from E2/v1.64.0, Explorer only; Workshop wiring and the cross-host parity test land in E3): the `@pspf/contracts` finder primitive (`searchRequirements`, `matchesRequirementFinderFilters`, `compareRequirementFinderRecords`) is the only requirement search/ordering implementation; result summaries state current assessment, evidence confidence, open actions, and material risk. `check:requirement-finder` asserts the primitive is defined once and wired into the Explorer requirements route without adding a new route (essentials surface budget stays frozen). Identical-fixture cross-host parity is asserted from E3.
