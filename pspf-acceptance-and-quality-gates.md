...
1. **Version and compatibility gate**: each slice advances all package versions and `PSPF_SLICE_VERSION` together (E0: all package versions and `PSPF_SLICE_VERSION` are `1.62.0`; E1: all package versions and `PSPF_SLICE_VERSION` are `1.63.0` … E8 = `1.70.0`); `VERSION_AXES` remain `1.15.0` for the entire programme; no entity, link, bundle, API, publication-policy, or Explorer schema change is introduced.
