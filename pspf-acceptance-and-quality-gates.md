...
8. **Journey gate**: an automated end-to-end walkthrough on the standard fixture proves search-or-browse → requirement explainer → evidence and action linking → save → visible posture/risk effect, without touching any Advanced control.
9. **Release-chain gate** (live from E0/v1.62.0): each slice adds its `e2e:v1.6x` script inheriting the complete previous chain; `release:readiness` targets the newest chain; ADR 0096's coverage entry upgraded from `manual` to `automated` now that `check:essentials-surface` and `e2e:v1.62` exist.
