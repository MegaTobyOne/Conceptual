...
5. **Relationship UI gate**: `@pspf/webview-shell.relationshipManagerHtml` renders escaped shared relationship actions and Shop uses it for assurance coverage quick actions.
6. **Consumer gate**: Shop and Workshop relationship command tests prevent reverting to ad hoc hardcoded link rules.
7. **Deferred-scope gate**: full relationship-manager rollout beyond the Shop proof point, edit-panel simplification, Pub full CRUD completion, Pub Explorer publication, and new schema-bearing relationship fields remain deferred.
8. **Regression gate**: `e2e:v1.29`, `check:ux-coverage`, `check:gates`, `package:check`, `validate:debug-workspace`, `lint`, `check:release-candidate`, and `typecheck` pass.
